...
Foundation landed and verified against the **live** chain (Rule 11). Test ratchet
**10 → 30**; **+1 TLA+ spec** (`specs/tla/SelectedParentReconcile.tla`, passes TLC);
**+1 tripwire** (`no-linear-chain-assumption`). `typecheck` + `build` + `lint` green.

| WP | State | Notes |
|----|-------|-------|
| WP-1.1 | ✅ code+tests | `chain.ts` + `rpc.ts` WS/HTTP clients; live `eth_chainId → 0x9d0c`. |
| WP-1.2 | ✅ code+tests | `rpc.ts` typed DAG layer (`getDagBlock` exposes selected/merge parents), `isFinal` boundary (99→false,100→true), live tips. |
| WP-1.3 | ✅ code · ⏳ DB-gated AC | Indexer ingests blocks/txs/**receipts/logs**/`dag_edges`(kind), blue_score-resume cursor, finality reconcile, reorg→`superseded` (never delete). Dry-mode live test reads head. **The 1000-block / resume / reorg ACs need Neon (D-Neon).** TLA+ spec proves finality-immutability + no-deletion. |
| WP-1.4 | ✅ code+migration | Schema reconciled (`dag_edges (child_hash,parent_hash,kind)`, `superseded`, `indexer_state`); migration `0000_flashy_salo.sql` generated; grains round-trip test. `drizzle-kit push` to Neon pending D-Neon. |
| WP-1.5 | ✅ code+tests | Dual-unit (X-4), `getBalance`, `exploreDag(blockHash)` walks the selected-parent spine + merge parents + finality; live `isContract` true/false. |
| WP-1.6 | ✅ code · ⏳ infer AC | `explainTransaction` (decodes ERC-20/721 Transfer/Approval), `audit_log` on every tool call, **Privy auth gate** on `/api/chat`. Streaming-answer AC needs the inference endpoint (D-Infer). |
| WP-1.7 | ✅ code+tests | All three crypto regimes round-trip (E2EE, hashed key, at-rest); auth env-gate unit-tested. Live Privy session AC needs D-Privy. |

**Blocked only on external provisioning** (see `PROVISIONING_HANDOFF.md`): D-Neon
(DATABASE_URL), D-Privy (app id/secret), D-Host (always-on worker box), D-Infer
(OpenAI-compatible endpoint). All code is drop-in: set the env and the gated live
tests + the continuous indexer activate with no code change.
