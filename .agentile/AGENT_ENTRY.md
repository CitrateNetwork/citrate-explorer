...
**CitrateScan** (`citrate-explorer`) — an AI-native BlockDAG explorer for the
Citrate Network (a better, agentic Etherscan).

- **Languages/toolchain:** TypeScript (Next.js 16 / React 19 / pnpm), Solidity
  (Foundry, solc 0.8.26).
- **Architecture (three runtimes):** an always-on **indexer worker** (WS
  `newHeads` → Neon), a **Vercel app** (UI + read API + AI agent + verification +
  MCP), and a **verify sandbox** (multi-version solc, S-4). See `../CLAUDE.md`.
- **Test invocation (ratchet):** `npx vitest run --reporter=json | jq '.numTotalTests'`
  (also in `coverage/baseline.json`). Live-RPC tests: `pnpm test:live`.
- **TLA+:** in scope — the DAG reorg/finality logic gets a spec in S-1.

For the finished product spec, see [PRODUCT_SPEC.md](PRODUCT_SPEC.md). For
canonical constants, see [CONFIG.md](CONFIG.md). For the build plan, see
[planset/2026-06-02-citrate-explorer-v1/PLANSET.md](planset/2026-06-02-citrate-explorer-v1/PLANSET.md).
