...
export async function POST(req: Request) {
  // Optional API key (higher limit) else anonymous per-IP identity, shared with
  // /api/v1. MCP tools/call hits live RPC, so the parsed operation count below
  // is charged against the limiter rather than treating a batch as one call.
  ...
  const limited = await checkRateLimit(limitId, perSec, undefined, batchSize);
  if (!limited.ok) {
