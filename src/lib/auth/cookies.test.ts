/**
 * FUA-EXPLORER-04 (SECREM-02 WP 7.1) — httpOnly auth-cookie plumbing.
 * Covers the cookie serializer attributes (HttpOnly + SameSite=Strict are the
 * load-bearing security properties), the request-cookie reader, and the
 * `tokenFromRequest` precedence in the session verifier (header wins, cookie
...
describe("tokenFromRequest (verifySession transport)", () => {
  ...
  it("falls back to the httpOnly session cookie", () => {
    const req = new Request("http://x/", {
      headers: { cookie: `${ID_COOKIE}=cookie-token` },
    });
