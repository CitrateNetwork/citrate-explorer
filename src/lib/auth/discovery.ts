...
/**
 * OIDC endpoint discovery (client-side). Per the auth handoff: NEVER hardcode
 * endpoint paths — read the authority's `/.well-known/openid-configuration`
 * (panva's authorization endpoint is `/auth`, not `/authorize`). Explicit env
 * overrides win; the panva defaults are only a last-resort fallback if discovery
 * is unreachable. Cached for the page lifetime.
 *
...
/** Citrate session bus (SSE) for the logout cascade (TD-5b). */
