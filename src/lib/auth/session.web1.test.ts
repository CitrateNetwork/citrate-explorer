/**
 * WEB-1 regression suite (SECREM-01, pre-audit 2026-06-09).
 *
 * The finding: `session.ts` defaulted NEXT_PUBLIC_AUTH_MODE to "mock", and
 * mock mode trusts the claims inside an UNSIGNED base64url token. A public
 * deploy with the env var unset accepted `Authorization: Bearer
 * base64url({"sub":"<victim>"})` and returned the victim's identity — full
 * account takeover with no key material.
 *
 * Red state (pre-fix): the "forged token" tests below FAILED — the forged
 * subject came back authenticated. Post-fix they pass: unset → oidc,
 * mock-in-production → disabled unless ALLOW_MOCK_AUTH=1.
 *
 * `session.ts` reads env at module load, so every scenario re-imports a
 * fresh copy (same pattern as session.oidc.test.ts).
 */
...
function forgedToken(sub: string, wallet: string): string {
  return Buffer.from(JSON.stringify({ sub, wallet_address: wallet })).toString(
    "base64url",
  );
}
...
function setEnv(env: Partial<Record<(typeof ENV_KEYS)[number], string | undefined>>) {
  for (const k of ENV_KEYS) {
    const v = env[k];
    // vi.stubEnv handles the read-only NODE_ENV typing and restores on
    // vi.unstubAllEnvs(); stub deletions as empty-then-delete fallback.
    if (v === undefined) {
      vi.stubEnv(k, "");
...
describe("WEB-1 — mode resolution matrix (resolveServerAuthMode)", () => {
  ...
  it("mock outside production stays available for dev", async () => {
    const { resolveServerAuthMode } = await freshSession();
    expect(
      resolveServerAuthMode({ NEXT_PUBLIC_AUTH_MODE: "mock", NODE_ENV: "test" }),
    ).toBe("mock");
    expect(
...
describe("WEB-1 — forged unsigned tokens are rejected end-to-end", () => {
  it("default deploy (mode unset): forged token yields NO identity", async () => {
    ...
    // requireOwner is what every authed route calls — must be null (→ 401).
  ...
  it("mock mode in production (no opt-in): dev header backdoor is dead too", async () => {
    ...
    const req = new Request("http://x", {
      headers: { "x-citrate-dev-address": VICTIM_WALLET },
    });
