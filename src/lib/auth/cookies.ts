/**
 * Auth-cookie plumbing (SECREM-02 WP 7.1 / FUA-EXPLORER-04). The OIDC id/access
 * tokens live in httpOnly cookies — never in web storage, never readable by
 * page script. Shared by the /api/auth/session route (set/clear) and
 * `verifySession` (read fallback when no Authorization header is present).
 *
...
/**
 * Serialize the httpOnly auth cookie. SameSite=Strict: the cookie rides only
 * on same-site requests (all the app's API fetches), never cross-site — the
 * CSRF mitigation for the cookie-auth migration. `Secure` outside dev.
 */
