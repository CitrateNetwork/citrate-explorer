...
-- HMAC-SHA256(key, API_KEY_PEPPER) instead of SHA-256(pepper ":" key). Hashes cannot be
-- converted without the raw keys (never stored), so every pre-HMAC key is revoked here.
